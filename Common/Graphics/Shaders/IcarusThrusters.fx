...
float4 IcarusThrusters(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0, float4 position : SV_Position) : COLOR0
{
    ...
    fire *= tex2D(image1, .1 * uv * 2 + float2(-time * .5, time)).r;
    ...
    color1 *= smoothstep(1,0,coords.x * coords.x * .25);
    color1 = lerp(saturate(color1), float4(0, 0, 0, 0), coords.x + (frac(time * 5) * 0.4)) * 3;
